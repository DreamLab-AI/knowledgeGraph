...
```json-ld
{
  ...
  "vc:plainGloss": "Search that matches on meaning rather than exact words, so a query about 'cars' still finds a page about 'automobiles'. It works by turning text into numerical fingerprints and finding the closest matches, so you get relevant results even when your wording differs from the document's.",
