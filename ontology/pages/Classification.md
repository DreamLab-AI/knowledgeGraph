...
- ### Definition
  - Classification is a core [[Supervised Learning]] task in which a model learns a mapping from input feature vectors to a finite set of discrete category labels, using a labelled training corpus to optimise decision boundaries or probabilistic scoring rules. It is one of the oldest and most thoroughly studied problems in [[Statistical Learning Theory]], grounded in theoretical traditions from [[Probabilistic Inference]], [[PAC Learning]], and [[Statistical Decision Theory]], with roots stretching from Bayes's 1763 theorem on conditional probabilities through to the deep [[Neural Network]] revolution of the 2010s and the foundation-model era of the 2020s. At the level of formal definition, a classifier is a function f: X → Y mapping an input space X (real-valued feature vectors, images, text sequences, graphs, or other structured data) to a label space Y = {y₁, y₂, …, yK} where K ≥ 2; the function is learned from a training set of (input, label) pairs by minimising an appropriate [[Loss Function]] — typically cross-entropy for probabilistic classifiers or hinge loss for margin-based models such as the [[Support Vector Machine]]. Classification underpins virtually all applied machine learning systems that produce categorical outputs, distinguishing it fundamentally from [[Regression]] (continuous predictions) and [[Clustering]] (unsupervised category discovery without predefined labels). The breadth of its application domain is extraordinary: an email classified as spam or not-spam, a radiological image classified as showing a tumour or not, a credit application classified as low or high risk, a spoken utterance classified as one of fifty thousand vocabulary items — all are instances of the same formal learning problem. Contemporary classification encompasses not only classical parametric and kernel-based approaches but also deep architectures that learn hierarchical feature representations end-to-end, [[Transfer Learning]] methods that adapt large pretrained models to new tasks with minimal labelled data, and [[Zero-Shot Learning]] paradigms enabled by multimodal foundation models such as CLIP and GPT-4, which perform classification through natural-language task descriptions without any task-specific training examples.

- ### Semantic Classification
  - owl-class:: ai:Classification
  - owl-role:: Concept | LearningParadigm | SupervisedTask
  - owl-inferred:: ai:PatternRecognitionTask, ai:CategoricalPredictionTask, ai:DecisionBoundaryLearner
  - belongs-to-domain:: [[AI-GroundedDomain]]
  - implemented-in-layer:: [[MachineLearningLayer]]

- ### Relationships
  - is-subclass-of:: [[Supervised Learning]], [[Machine Learning]], [[Pattern Recognition]]
  - has-part:: [[Binary Classification]], [[Multi-Class Classification]], [[Multi-Label Classification]], [[Ordinal Classification]], [[Hierarchical Classification]], [[Classification Threshold]]
  - requires:: [[Supervised Learning]], [[Labelled Dataset]], [[Feature Engineering]], [[Loss Function]], [[Training Data]], [[Model Evaluation]]
  - enables:: [[Object Detection]], [[Sentiment Analysis]], [[Image Recognition]], [[Spam Filtering]], [[Medical Diagnosis AI]], [[Fraud Detection]], [[Robotic Perception]], [[Text Classification]], [[Named Entity Recognition]], [[Intent Detection]]
  - implements:: [[PAC Learning]], [[Statistical Decision Theory]], [[Empirical Risk Minimisation]]
  - depends-on:: [[Training Data]], [[Cross Validation]], [[Hyperparameter Tuning]], [[Probability Calibration]], [[Classification Threshold]]
  - supports:: [[AI Governance]], [[AI Fairness]], [[Explainable AI]], [[Knowledge Graph]], [[Autonomous Systems]]
  - uses:: [[Decision Tree]], [[Support Vector Machine]], [[Neural Network]], [[Random Forest]], [[Gradient Boosted Trees]], [[Logistic Regression]], [[Naive Bayes]], [[K-Nearest Neighbours]], [[Transformer Architecture]], [[Convolutional Neural Network]]
  - contrasts-with:: [[Regression]], [[Clustering]], [[Anomaly Detection]], [[Reinforcement Learning]], [[Generative Model]]
  - related-to:: [[Statistical Learning Theory]], [[Probabilistic Inference]], [[Transfer Learning]], [[Meta-Learning]], [[Representation Learning]], [[Zero-Shot Learning]], [[Few-Shot Learning]], [[Class Imbalance]], [[Confusion Matrix]], [[ROC Curve]], [[Conformal Prediction]], [[Active Learning]], [[Semi-Supervised Learning]], [[Continual Learning]], [[AutoML]], [[Ensemble Method]]
  - bridges-to:: [[Knowledge Graph]], [[Robotic Perception]], [[Computer Vision]], [[Natural Language Processing]], [[Bioinformatics]]
  - standardized-by:: [[ISO/IEC 22989]], [[ISO/IEC 23053]], [[NIST AI RMF]], [[EU AI Act]]
...
- ### Content
  ...
  ## Key Terminology
    ...
    | Term | Definition |
    |------|-----------|
    | Classifier | A function f: X → Y (or f: X → ΔY for probabilistic classifiers) mapping inputs to discrete or probabilistic class assignments |
    | Decision boundary | The subset of X where the classifier changes its prediction; a hyperplane for linear classifiers, an arbitrary surface for non-linear models |
    | Feature space | The d-dimensional real vector space X ⊆ ℝᵈ in which input examples are represented |
    | Hypothesis class | The set H of possible classifiers from which the learning algorithm selects; characterised by VC dimension or Rademacher complexity |
    | Empirical risk | The average training loss over the training sample; minimised by most learning algorithms subject to regularisation |
    | Generalisation error | The expected loss on unseen test data drawn from the same distribution as training data; the quantity we actually care about |
    | VC dimension | Vapnik-Chervonenkis dimension: the largest set of points that the hypothesis class can shatter (correctly classify in all 2ᵈ ways); characterises sample complexity |
    | Inductive bias | The implicit assumptions built into a learning algorithm that determine which hypothesis it selects when multiple hypotheses fit the training data |
    | Overfitting | When a classifier memorises training data but fails to generalise; detected by large gap between training and test accuracy |
    | Underfitting | When a classifier is too simple to capture the true decision boundary; detected by poor performance on both training and test data |
    | Softmax | The multi-class generalisation of the sigmoid function; converts a K-dimensional real vector (logits) to a probability distribution summing to 1 |
    | Cross-entropy loss | The standard loss function for classification: L = −Σᵢ yᵢ log p̂ᵢ; equals the KL divergence between the true label distribution and the predicted distribution |
    | Hinge loss | The SVM loss: max(0, 1 − yf(x)); penalises predictions within the margin; leads to sparse support vector solutions |
    | Class imbalance | The condition where one or more classes have far fewer training examples than others; requires special handling via resampling, class weighting, or [[Classification Threshold]] adjustment |
    | Precision / Recall | Threshold-dependent evaluation metrics; precision = TP/(TP+FP), recall = TP/(TP+FN); controlled via [[Classification Threshold]] |
    | F1-score | Harmonic mean of precision and recall; 2·P·R/(P+R); the standard single-number summary for imbalanced classification evaluation |
    | AUC-ROC | Area Under the [[ROC Curve]]; threshold-independent discriminative performance metric; probability that a random positive scores higher than a random negative |
    | Transfer learning | Adapting a model pretrained on a large source task to a new target classification task; central to modern NLP (BERT, GPT) and vision (ImageNet pretraining) |
    | Zero-shot learning | Classifying examples from unseen classes without any labelled training examples for those classes; enabled by semantic embeddings or natural language descriptions |
    | Conformal prediction | A framework for producing prediction sets with guaranteed coverage probability; provides distribution-free uncertainty quantification for classifiers |
    | Model calibration | The property that predicted probabilities match empirical frequencies; p̂ = 0.7 should be correct ~70% of the time; prerequisite for principled [[Classification Threshold]] setting |
...
- ### Provenance
  - sources:: Bishop, C.M. — Pattern Recognition and Machine Learning (2006); Hastie, Tibshirani, Friedman — The Elements of Statistical Learning (2009); ISO/IEC 22989:2022; ISO/IEC 23053:2022; NIST AI RMF 1.0 (2023); EU AI Act (2024); scikit-learn documentation; https://labelyourdata.com/articles/image-classification-models; https://encord.com/blog/machine-learning-image-classification-guide/; https://www.ucl.ac.uk/engineering/computer-science/research/research-themes/machine-learning; https://machinelearningjobs.co.uk/career-advice/top-10-best-uk-universities-for-machine-learning-degrees-2025-guide; https://zylos.ai/research/2026-02-05-ai-bias-fairness; Krizhevsky et al. (2012) AlexNet; Dosovitskiy et al. (2021) ViT; Radford et al. (2021) CLIP; Chicco & Jurman (2020) MCC
