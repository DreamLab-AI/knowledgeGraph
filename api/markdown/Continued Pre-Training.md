...
```clojure
(Declaration (Class :ContinuedPreTraining))
(SubClassOf :ContinuedPreTraining :TrainingTechnique)
(SubClassOf :ContinuedPreTraining
  (ObjectSomeValuesFrom :followsPhase :PreTraining))
(SubClassOf :ContinuedPreTraining
  (ObjectSomeValuesFrom :precedesPhase :FineTuning))
(SubClassOf :ContinuedPreTraining
  (ObjectSomeValuesFrom :trainsOn :DomainSpecificUnlabelledData))
(SubClassOf :ContinuedPreTraining
  (ObjectSomeValuesFrom :enables :DomainAdaptation))
(SubClassOf :ContinuedPreTraining
  (ObjectSomeValuesFrom :maintains :GeneralCapabilities))
(SubClassOf :ContinuedPreTraining
  (ObjectSomeValuesFrom :prevents :CatastrophicForgetting))

(AnnotationAssertion rdfs:comment :ContinuedPreTraining
  "Intermediate training phase adapting pre-trained models to specific domains through additional pre-training on domain-relevant unlabelled data"@en)
(AnnotationAssertion :hasAcademicSource :ContinuedPreTraining
  "Transfer learning literature; arXiv:2411.01195 (2024)")
```
