...
Node (BC-0072) refers to a component of the metaverse ecosystem focusing on node (bc-0072).
...
Ontology(<http://narrativegoldmine.com/blockchain/BC-0072>
...
5. **Performance Property**: Resource requirements vary by node type - [[full nodes]] require substantial storage (600+ GB for Bitcoin, 1+ TB for Ethereum), whilst [[light nodes]] minimise resource usage through [[SPV]] (Simplified Payment Verification)
...
- **Definition**: Nodes authorised to propose new blocks and participate in [[consensus mechanism]]
...
- **Application**: Running full nodes distributes blockchain validation across independent operators, preventing centralised control
...
3. **Node Operators**: Enforce [[consensus rules]] that miners must follow, optimise [[trust minimization]], protect network from protocol changes that might undermine security or usability
...
**Key Innovation**: [Stablesats](https://stablesats.com/) - Algorithmic dollar stability using exclusively Lightning Bitcoin, without centralised stablecoin reliance.
...
Despite Ethereum's **400,000+ validator nodes** (post-Merge) creating robust consensus decentralization, the vast majority of users and applications accessed the network through **centralised RPC providers** (Alchemy, Infura, QuickNode). This created a **centralization paradox**:
- **Consensus Layer**: Highly decentralized (400,000+ validators)
- **Access Layer**: Highly centralised (3-4 dominant providers)
...
Major node providers operated data centres predominantly in **United States and Europe** (AWS, Google Cloud, Microsoft Azure regions):
...
- Demonstrated that centralised infrastructure created de facto censorship capability even on ostensibly censorship-resistant blockchains
...
Awareness of infrastructure centralization risks catalysed **self-hosted node movement** through 2024-2025:
...
Protocols emerged as alternatives to centralised RPC providers:
...
1. **Permanent Infrastructure Centralization**
   ...
   - Decentralized consensus layers masked by centralised access layers
...
- **Protocols**: Supports UDP, TCP, [[WebSockets]]. DNS only used briefly during bootstrap to minimise metadata leakage.
...
### Goals and Features
- **Decentralization**: Enable decentralized apps without reliance on centralised corporate systems (cloud providers, centralised servers)
- **Privacy**: IP address privacy through encrypted multi-hop routing (similar to [[Tor]] but optimised for mobile)
...
Key features include strong cryptography, ability to run on variety of platforms, distributed and replicated data storage, and private routing to provide IP privacy. The decentralized design aims to avoid issues with centralised and corporate controlled systems.
