---
public: true
---

# Lump of Labor Fallacy
```json-ld
{
  "@context": "https://narrativegoldmine.com/context/v1.jsonld",
  "@id": "urn:visionflow:page:lump-of-labor-fallacy",
  "@type": "Page",
  "vc:slug": "lump-of-labor-fallacy",
  "title": "Lump of Labor Fallacy",
  "vc:public": true,
  "vc:outboundWikilinks": ["Demand Elasticity", "Economic Impact of AI", "Labor Supply"],
  "vc:schemaVersion": 2
}
```

```json-ld
{
  "@context": "https://narrativegoldmine.com/ns/v2.jsonld",
  "@id": "urn:ngm:class:lump-of-labor-fallacy",
  "@type": "Class",
  "label": "Lump of Labor Fallacy",
  "definition": "An economic fallacy that assumes a fixed total amount of work exists, leading to the incorrect conclusion that technological increases in labor supply or productivity necessarily result in unemployment.",
  "domain": "economics",
  "maturity": "draft",
  "quality": 0.35,
  "subClassOf": [{"@id": "urn:ngm:class:economic-impact-of-ai", "label": "Economic Impact of AI"}],
  "relations": {
    "relatedTo": [{"@id": "urn:ngm:class:labor-supply", "label": "Labor Supply"}, {"@id": "urn:ngm:class:demand-elasticity", "label": "Demand Elasticity"}, {"@id": "urn:ngm:class:artificial-intelligence", "label": "Artificial Intelligence"}]
  },
  "provenance": {
    "source": "podcast-knowledge-ingest",
    "created": "2026-08-24",
    "episode": "the-new-jobs-ai-will-create.md"
  }
}
```

- ### Overview
  - **[Industry analysis]** The host identifies a 'lump of labor fallacy' in current AI job narratives, arguing that assuming constant demand while AI increases labor supply ignores the expansionary nature of economies and demand elasticity. *(Source: AI Daily Brief Host, via AI Daily Brief, 2026-08-24)*
- ### Relationships
- ### Provenance
