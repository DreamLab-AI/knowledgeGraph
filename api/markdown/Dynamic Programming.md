...
- ### Content
  ...
  **Q-Learning Algorithm (Model-Free Approximate DP):**
  ...
  - **Dimension reduction**: sometimes a 2D state can be collapsed to 1D by observing that one dimension is always determined by the recurrence [private]
  Common state representations in competitive programming and algorithmic interviews:
  - Prefix [private] (LCS, Edit Distance, LIS): state = (i, j) for two prefixes, or (i) for a single prefix
  ...
  - **Off-by-one audit**: DP is notoriously prone to off-by-one errors in loop bounds, [private] mappings, and base case initialisation
  ...
  **Maximum Subarray Sum (Kadane's Algorithm):** dp[i] = max(A[i], dp[i-1] + A[i]). The maximum subarray sum ending at [private] i is either A[i] alone or extending the best ending at i-1. Time O(n), space O(1). One of the simplest DP algorithms; extremely useful in financial time-series analysis (maximum profit window), signal processing, and computer vision (2D variant finds the maximum-sum rectangular submatrix in O(n^4) or O(n^3) with Kadane).
  ...
  **Quantum-Accelerated Dynamic Programming:**
  ...
  - Quantum speedups for specific DP subproblems: O(sqrt(n)) Grover search within each DP state expansion [private] yield O(sqrt(n)·S·T) algorithms for DP with a search component.
