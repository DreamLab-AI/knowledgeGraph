...
- ### Content
  - The theoretical foundations of network communication were established by Claude Shannon's 1948 information theory paper, which defined channel capacity and the maximum error-free data rate for a noisy channel. Practical packet-switching networks emerged with ARPANET (1969), which demonstrated that messages [private] be broken into independently routed packets and reassembled — a paradigm shift from circuit-switched telephony. Vint Cerf and Bob Kahn formalised the TCP/IP protocol suite in 1974, creating the interoperability layer that eventually became the internet. The OSI seven-layer reference model (ISO 7498, 1984) provided a vendor-neutral framework for protocol design and standardisation.
