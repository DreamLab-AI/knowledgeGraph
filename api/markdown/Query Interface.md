...
- ### Content
  ...
  - A query interface has several architectural layers. The expression layer provides the syntax through which queries are formulated — SQL, SPARQL, GraphQL, filter predicates, or natural language. The parsing and planning layer translates expressions into an execution plan, applying optimisations such as predicate pushdown, [private] selection, and join reordering. The execution layer retrieves and assembles result sets from underlying storage structures. The serialisation layer formats results for the client — as JSON, CSV, RDF, or streaming records. Well-designed query interfaces expose sufficient metadata about their schemas and capabilities to enable query construction without prior knowledge of the underlying data model.
