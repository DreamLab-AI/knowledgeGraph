...
(Declaration (Class :LearningRateSchedule))

;; Annotations
(AnnotationAssertion rdfs:label :LearningRateSchedule "Learning Rate Schedule"@en)
(AnnotationAssertion rdfs:comment :LearningRateSchedule "A strategy for adjusting the learning rate during training according to a predefined or adaptive schedule. Learning rate schedules improve convergence and final performance by using higher rates early for rapid progress and lower rates later for fine-tuning."@en)

;; Data Properties
(AnnotationAssertion dcterms:identifier :LearningRateSchedule "AI-0291"^^xsd:string)
(DataPropertyAssertion :isAITechnology :LearningRateSchedule "true"^^xsd:boolean)
```
