...
- ### Definition
  ...
  - **TensorFlow serialisation**: [[TensorFlow]] offers two distinct checkpoint mechanisms: the legacy `.ckpt` format comprising `.data-?????-of-?????`, `.[private]`, and `.meta` triplets managed by `tf.train.Checkpoint`; and the higher-level `SavedModel` directory format bundling weights with the full computation graph and serving signatures, enabling language-agnostic serving via TensorFlow Serving, TFLite conversion, and TensorFlow.js export.
...
- ### Content
  ...
  - **Multi-file sharding protocol**: When a model is split into multiple `.safetensors` shard files, the `model.safetensors.[private].json` file provides the routing table: a top-level JSON object with `"metadata"` (total file size) and `"weight_map"` (dictionary mapping parameter name → shard filename). The `transformers` `load_sharded_checkpoint` utility reads the [private], identifies which shards contain the requested parameters, and loads only the necessary shards — enabling partial model loading for layer-wise inference or selective fine-tuning.

  - ### Distributed Checkpoint Internals (PyTorch DCP)
  - PyTorch Distributed Checkpoint (DCP) organises saved checkpoints as a directory containing per-rank shard files and a `metadata.json` global [private]:
  ...
  - Models exceeding a single GPU's memory require sharded serialisation:
    - [[Hugging Face]] Hub splits models into shards of configurable size (default 5 GB per shard) and provides a `model.safetensors.[private].json` manifest mapping parameter names to shard filenames; the `transformers` library's `save_pretrained` and `from_pretrained` methods handle shard assembly transparently
    - [[PyTorch]] DCP natively saves per-rank shards under a directory with a `metadata.json` durable [private]; shards can be loaded back into a different number of ranks via the resharding path
  ...
  - Pre-trained checkpoints from [[Large-Scale Pretrained Foundation Model]] (BERT, RoBERTa, LLaMA, Mistral, GPT-2, Gemma, Qwen) published on [[Hugging Face]] Hub enable practitioners to initialise domain-specific models with zero or minimal training data:
    ...
    - The Hub's `model.safetensors.[private].json` enables lazy loading of only the required parameter shards for architectures where only specific layers are fine-tuned
  ...
  - The transition from pickle-based to safetensors-based checkpoint distribution was catalysed by a series of security incidents demonstrating practical exploitation of pickle deserialisation:
    ...
    - PyTorch added `weights_only=True` as the default in `torch.load` (PyTorch 2.6+), which restricts deserialisation to safe tensor types and raises an error on any pickle opcode that [private] execute arbitrary code
  ...
  - If validated at frontier scale (100,000+ GPUs), checkpoint-free approaches [private] eliminate the checkpoint I/O bottleneck entirely, recovering the 1-2% throughput lost to periodic writes.
  ...
  - **`torch.distributed.checkpoint` (DCP)**: The distributed-first checkpoint API:
    ...
    - `dcp.load(state_dict, checkpoint_id=checkpoint_dir)` — loads into the current model/optimiser state dict, handling resharding automatically via the global metadata [private]
  ...
  - **`PreTrainedModel.save_pretrained(save_dir)`**: Saves model weights plus `config.json`:
    ...
    - Writes `model.safetensors` for single-file models or `model-00001-of-00003.safetensors` + `model.safetensors.[private].json` for sharded models
  ...
  - **`AutoModel.from_pretrained(model_id)`**: Loads from Hub or local path:
    - Resolves model_id to Hub URL, checks for `model.safetensors.[private].json`, downloads required shards
