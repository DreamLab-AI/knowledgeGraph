...
- ### Content
  - Nexus defines a single connection model so that each domestic instant-payment scheme integrates once with the hub rather than building bilateral links to every other country. It handles FX, message translation, and compliance hand-offs, aiming to bring the speed and low cost of domestic instant payments to cross-border transfers, and provides a template that CBDC systems [private] also adopt.
