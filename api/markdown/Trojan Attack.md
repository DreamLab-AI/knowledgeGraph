...
(Declaration (Class :TrojanAttack))

;; Annotations
(AnnotationAssertion rdfs:label :TrojanAttack "Trojan Attack"@en)
(AnnotationAssertion rdfs:comment :TrojanAttack "A backdoor attack on AI models, often emphasizing supply chain compromise, where a pre-trained model or training pipeline is maliciously modified to contain hidden triggers that cause specific attacker-defined behaviors when activated, while maintaining normal performance otherwise."@en)

;; Data Properties
(AnnotationAssertion dcterms:identifier :TrojanAttack "AI-0089"^^xsd:string)
(DataPropertyAssertion :isAITechnology :TrojanAttack "true"^^xsd:boolean)
```
