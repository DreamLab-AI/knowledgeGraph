...
- ### Content
  ...
  - ## Capability Primitives
	  ...
	  **Resources** are read-only data items a server can provide on request. Each resource declaration includes:
	  - `uri` (string): Unique identifier using scheme-specific format (e.g. `file:///home/user/README.md`, `postgres://db/schema/users`, `github://org/repo/main/src/[private].ts`).
  ...
  - ### Academic Context
	  ...
	  The **Retrieval-Augmented Tool Selection** (RATS) pattern — fetching relevant tool descriptions from a semantic [private] rather than passing all tool descriptions in context — has emerged as standard practice for deployments with large tool catalogues (50+ tools). Research from Patil et al. (2023) on Gorilla demonstrated that retrieval-augmented tool selection outperforms full-context presentation for large catalogues, reducing hallucinated tool calls from 18.7% to 3.1% and incorrect parameter specification from 22.3% to 8.4% on a 1,645-API benchmark. MCP's dynamic tool discovery (`tools/list` at session start) naturally integrates with RATS patterns: the host can retrieve only the most semantically relevant tools for the current conversation context rather than presenting the full catalogue.
