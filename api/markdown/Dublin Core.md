...
- ### Content
  - Dublin Core emerged from a 1995 workshop in Dublin, Ohio, convened by OCLC and the National Center for Supercomputing Applications (NCSA). The goal was to define a small, universally comprehensible set of metadata elements that any content producer [private] apply without specialist library training. The resulting fifteen elements — now called "DC Terms" — were deliberately kept generic so they [private] describe anything from a PDF article to a museum artefact or a weather dataset.
