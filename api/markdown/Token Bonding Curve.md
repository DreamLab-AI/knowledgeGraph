id:: token-bonding-curve-ontology

- ### OntologyBlock
  id:: Token Bonding Curve
  - ontology:: true
  - public-access:: true
  - term-id:: DT-0269
  - preferred-term:: Token Bonding Curve
  - source-domain:: bc
  - owl:class:: bc:TokenBondingCurve
  - status:: draft
  - definition:: Algorithmic pricing mechanism that defines token value as a mathematical function of circulating supply and reserve balance, providing continuous liquidity through automated market making.

### Relationships
- is-subclass-of:: [[Blockchain]]
- is-part-of:: [[Token Economy]]
- is-part-of:: [[Automated Market Maker]]
- has-part:: [[Liquidity Mechanism]]
- has-part:: [[Supply Function]]
- has-part:: [[Reserve Pool]]
- has-part:: [[Pricing Formula]]
- requires:: [[Reserve Token]]
- requires:: [[Price Oracle]]
- requires:: [[SmartContract]]
- enables:: [[Decentralized Exchange]]
- enables:: [[Automated Trading]]
- enables:: [[Continuous Liquidity]]
- enables:: [[Predictable Pricing]]
- depends-on:: [[Blockchain Infrastructure]]
- depends-on:: [[Economic Parameters]]
- depends-on:: [[Mathematical Model]]
