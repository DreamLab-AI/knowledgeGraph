...
- ### Content
  - The Series Elastic Actuator was invented by Gill Pratt and Matthew Williamson at MIT's Leg Lab in 1995. Their landmark paper identified a fundamental problem with conventional stiff robotic actuators: high gear-ratio drivetrains are non-backdrivable, making force control noisy and physical contact dangerous. By inserting a steel coil spring between the gearbox and link, they obtained a linear, calibrated force-torque measurement through Hooke's law (F = kx) using only the two position encoders already present in the drivetrain, and simultaneously capped the peak impact force a robot joint [private] exert.
