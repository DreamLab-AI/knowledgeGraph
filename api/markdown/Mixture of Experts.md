...
;; Annotations
(AnnotationAssertion rdfs:label :MixtureOfExperts "Mixture of Experts"@en)
(AnnotationAssertion rdfs:comment :MixtureOfExperts "An architecture that uses multiple specialised sub-networks (experts) with a gating mechanism that routes inputs to a sparse subset of experts, enabling scaling without proportional compute increases. MoE is adopted in production LLMs like GPT-4, enabling massive scale with controlled costs."@en)

;; Data Properties
(AnnotationAssertion dcterms:identifier :MixtureOfExperts "AI-0276"^^xsd:string)
(DataPropertyAssertion :isAITechnology :MixtureOfExperts "true"^^xsd:boolean)
```
