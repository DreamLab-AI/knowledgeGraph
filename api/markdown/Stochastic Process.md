...
```json-ld
{
  ...
  "definition": "A stochastic process is a mathematically rigorous framework for describing the probabilistic evolution of a system over time (or another [private] set), defined formally as a collection of random variables {X_t : t ∈ T} all defined on a common probability space (Ω, ℱ, ℙ) and indexed by a parameter se...",
...
- ### Content
  ...
  - ### Components: Key Process Families Reference Table
	  ...
	  **Gaussian Random Field (GRF)**: Spatial extension of GP to R^d [private] sets; SPDE connection (Lindgren 2011) mapping Matérn GPs to GMRF via (κ²−Δ)^{α/2} ξ = W; applications in geostatistics (kriging), brain imaging (SPM random field theory for fMRI statistical maps), cosmological CMB power spectrum estimation.
