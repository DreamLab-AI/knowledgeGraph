...
# Epoch
		  ...
		  - **Term ID**: AI-0049
		  - **Type**: AICapability
		  - **Classification**: Training Metric
		  - **Domain**: MLDomain
		  ...
		  **Source**: ISO/IEC 22989:2022 (Training) + Academic consensus - Authority Score: 0.92
		  ...
		  - **Full Dataset Pass**: Processes all training examples once
		  - **Iterative Training**: Multiple epochs improve model performance
		  - **Monitoring Unit**: Training progress tracked by epoch
		  - **Convergence Indicator**: Model converges over epochs
		  - **Early Stopping**: Training may stop after optimal epoch
		  
		  ## Relationships
		  
		  ### Parent Classes
		  - **Training Metric**: Epoch is a unit of training progress
		  ...
		  ```clojure
		  ;; Epoch Ontology (OWL Functional Syntax)
		  ;; Term ID: AI-0049
		  ;; Domain: MLDomain | Layer: AlgorithmicLayer
		  
		  (Declaration (Class :Epoch))
		  
		  ;; Core Classification
		  (SubClassOf :Epoch :TrainingMetric)
		  (SubClassOf :Epoch :IterationUnit)
		  
		  ;; Process Characteristics
		  (SubClassOf :Epoch
		    (ObjectSomeValuesFrom :represents :CompleteDatasetPass))
		  (SubClassOf :Epoch
		    (ObjectSomeValuesFrom :comprisedOf :MultipleBatches))
		  (SubClassOf :Epoch
		    (ObjectSomeValuesFrom :measures :TrainingProgress))
		  
		  ;; Relationship to Training
		  (SubClassOf :Epoch
		    (ObjectSomeValuesFrom :usedIn :Training))
		  (SubClassOf :Epoch
		    (ObjectSomeValuesFrom :indicates :Convergence))
		  
		  ;; Annotations
		  (AnnotationAssertion rdfs:label :Epoch "Epoch"@en)
		  (AnnotationAssertion rdfs:comment :Epoch
		    "One complete pass through entire training dataset during model training"@en)
		  (AnnotationAssertion :isoReference :Epoch "ISO/IEC 22989:2022, Training terminology")
		  (AnnotationAssertion :authorityScore :Epoch "0.92"^^xsd:float)
		  (AnnotationAssertion :priorityLevel :Epoch "1"^^xsd:integer)
		  
		  ;; Data Properties
		  (DataPropertyAssertion :processesEntireDataset :Epoch "true"^^xsd:boolean)
		  (DataPropertyAssertion :isIterativeUnit :Epoch "true"^^xsd:boolean)
		  (DataPropertyAssertion :supportsEarlyStopping :Epoch "true"^^xsd:boolean)
		  
		  ;; Property Declarations
		  (Declaration (ObjectProperty :comprisedOf))
		  (ObjectPropertyDomain :comprisedOf :Epoch)
		  (ObjectPropertyRange :comprisedOf :MultipleBatches)
		  
		  (Declaration (DataProperty :processesEntireDataset))
		  (DataPropertyDomain :processesEntireDataset :Epoch)
		  (DataPropertyRange :processesEntireDataset xsd:boolean)
		  ```
		  </details>
