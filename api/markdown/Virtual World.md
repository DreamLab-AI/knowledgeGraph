id:: virtual-world-ontology

- ### OntologyBlock
  id:: Virtual World
  - ontology:: true
  - public-access:: true
  - term-id:: DT-0314
  - preferred-term:: Virtual World
  - source-domain:: mv
  - owl:class:: mv:VirtualWorld
  - status:: deprecated
  - definition:: A self-contained digital environment with persistent state, spatial properties, user interaction capabilities, and internal rules that simulate physical or fantastical worlds, providing a shared space for multiple users to interact with each other and digital objects.

### Relationships
- is-subclass-of:: [[Metaverse]]
- is-part-of:: [[Metaverse Platform]]
- is-part-of:: [[Metaverse]]
- has-part:: [[Scene Graph]]
- has-part:: [[Tourism Metaverse]]
- has-part:: [[Object Persistence]]
- has-part:: [[User Representation]]
- has-part:: [[World Space]]
- has-part:: [[Digital Real Estate]]
- has-part:: [[Economic System]]
- has-part:: [[Social System]]
- has-part:: [[Physics Engine]]
- requires:: [[Asset Management]]
- requires:: [[Database System]]
- requires:: [[Network Protocol]]
- requires:: [[Authentication Service]]
- requires:: [[3D Rendering Engine]]
- enables:: [[Social Interaction]]
- enables:: [[Digital Economy]]
- enables:: [[Virtual Society]]
- enables:: [[Creative Expression]]
- enables:: [[Collaborative Work]]
- depends-on:: [[Client Application]]
- depends-on:: [[Content Delivery Network]]
- depends-on:: [[Server Infrastructure]]

### DEPRECATED
### DEPRECATED
**This entity has been merged into [[VirtualWorld]]**
**Status**: deprecated
**Merge Date**: 2025-11-23
**Reason**: Duplicate entity consolidation
