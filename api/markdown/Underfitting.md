...
# Underfitting
		  ...
		  **Source**: ISO/IEC 22989:2022 (Model quality) + Academic consensus - Authority Score: 0.93
		  
		  ### Operational Characteristics
		  - **Low Training Performance**: Poor results on training data
		  - **Low Test Performance**: Poor results on test data
		  - **Insufficient Complexity**: Model too simple for task
		  - **High Bias**: Model makes strong simplifying assumptions
		  - **Solution**: Increase capacity or training duration
		  
		  ## Relationships
		  
		  ### Parent Classes
		  - **Training Problem**: Underfitting is a training pathology
		  ...
		  ```clojure
		  ;; Underfitting Ontology (OWL Functional Syntax)
		  ;; Term ID: AI-0055
		  ;; Domain: MLDomain | Layer: AlgorithmicLayer
		  
		  (Declaration (Class :Underfitting))
		  
		  ;; Core Classification
		  (SubClassOf :Underfitting :TrainingProblem)
		  (SubClassOf :Underfitting :AIRisk)
		  
		  ;; Performance Characteristics
		  (SubClassOf :Underfitting
		    (ObjectSomeValuesFrom :exhibitsLow :TrainingPerformance))
		  (SubClassOf :Underfitting
		    (ObjectSomeValuesFrom :exhibitsLow :TestPerformance))
		  (SubClassOf :Underfitting
		    (ObjectSomeValuesFrom :failsToCapture :UnderlyingPatterns))
		  
		  ;; Causality
		  (SubClassOf :Underfitting
		    (ObjectSomeValuesFrom :causedByInsufficient :ModelCapacity))
		  (SubClassOf :Underfitting
		    (ObjectSomeValuesFrom :resultOfTooSimple :ModelArchitecture))
		  (SubClassOf :Underfitting
		    (ObjectSomeValuesFrom :orCausedByInsufficient :Training))
		  
		  ;; Solution
		  (SubClassOf :Underfitting
		    (ObjectSomeValuesFrom :addressedByIncreasing :ModelComplexity))
		  (SubClassOf :Underfitting
		    (ObjectSomeValuesFrom :orByExtending :TrainingDuration))
		  
		  ;; Contrast
		  (DisjointClasses :Underfitting :Overfitting)
		  
		  ;; Annotations
		  (AnnotationAssertion rdfs:label :Underfitting "Underfitting"@en-GB)
		  (AnnotationAssertion rdfs:comment :Underfitting
		    "Model too simple to capture underlying patterns, resulting in poor performance on both training and test data"@en)
		  (AnnotationAssertion :isoReference :Underfitting "ISO/IEC 22989:2022")
		  (AnnotationAssertion :authorityScore :Underfitting "0.93"^^xsd:float)
		  (AnnotationAssertion :priorityLevel :Underfitting "1"^^xsd:integer)
		  
		  ;; Data Properties
		  (DataPropertyAssertion :lowTrainingPerformance :Underfitting "true"^^xsd:boolean)
		  (DataPropertyAssertion :lowTestPerformance :Underfitting "true"^^xsd:boolean)
		  (DataPropertyAssertion :indicatesHighBias :Underfitting "true"^^xsd:boolean)
		  ```
		  </details>
