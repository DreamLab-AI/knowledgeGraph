...
```clojure
;; Class Declaration
(Declaration (Class :LimitedRiskAI))

;; Subclass Axiom
(SubClassOf :LimitedRiskAI :AISystem)

;; Defining Characteristic
(SubClassOf :LimitedRiskAI
  (ObjectSomeValuesFrom :subjectOnlyTo :TransparencyObligation))

(SubClassOf :LimitedRiskAI
  (ObjectAllValuesFrom :exemptFrom :ConformityAssessment))

(SubClassOf :LimitedRiskAI
  (ObjectAllValuesFrom :exemptFrom :TechnicalDocumentationRequirement))

(SubClassOf :LimitedRiskAI
  (ObjectAllValuesFrom :exemptFrom :CEMarkingRequirement))

;; Four Covered System Types
(DisjointUnion :LimitedRiskAI
  :ChatbotAI
  :EmotionRecognitionAI
  :BiometricCategorisationAI
  :SyntheticContentGenerationAI)

;; Type 1: Chatbots
(Declaration (Class :ChatbotAI))
(SubClassOf :ChatbotAI :LimitedRiskAI)

(SubClassOf :ChatbotAI
  (ObjectSomeValuesFrom :interactsWith :NaturalPerson))

(SubClassOf :ChatbotAI
  (ObjectSomeValuesFrom :mustInform :UserOfAIInteraction))

(SubClassOf :ChatbotAI
  (ObjectSomeValuesFrom :disclosureAt :InteractionStart))

;; Chatbot Exceptions
(Declaration (Class :ExemptChatbot))
(SubClassOf :ExemptChatbot :ChatbotAI)

(SubClassOf :ExemptChatbot
  (ObjectSomeValuesFrom :whenObviousFrom :Circumstances))

(Declaration (Class :LawEnforcementChatbot))
(SubClassOf :LawEnforcementChatbot :ExemptChatbot)

(SubClassOf :LawEnforcementChatbot
  (ObjectSomeValuesFrom :authorisedFor :CriminalOffenceDetection))

;; Type 2: Emotion Recognition
(Declaration (Class :EmotionRecognitionAI))
(SubClassOf :EmotionRecognitionAI :LimitedRiskAI)

(SubClassOf :EmotionRecognitionAI
  (ObjectSomeValuesFrom :identifies :HumanEmotions))

(SubClassOf :EmotionRecognitionAI
  (ObjectSomeValuesFrom :mustInform :ExposedPersons))

;; Type 3: Biometric Categorisation
(Declaration (Class :BiometricCategorisationAI))
(SubClassOf :BiometricCategorisationAI :LimitedRiskAI)

(SubClassOf :BiometricCategorisationAI
  (ObjectSomeValuesFrom :categorisesBasedOn :BiometricData))

(SubClassOf :BiometricCategorisationAI
  (ObjectAllValuesFrom :notFor :UniqueIdentification))

(SubClassOf :BiometricCategorisationAI
  (ObjectSomeValuesFrom :mustInform :CategorisedPersons))

;; Type 4: Synthetic Content (Deepfakes)
(Declaration (Class :SyntheticContentGenerationAI))
(SubClassOf :SyntheticContentGenerationAI :LimitedRiskAI)

(SubClassOf :SyntheticContentGenerationAI
  (ObjectSomeValuesFrom :generates :SyntheticContent))

(SubClassOf :SyntheticContentGenerationAI
  (ObjectSomeValuesFrom :mustLabel :GeneratedContent))

;; Synthetic Content Types
(Declaration (Class :SyntheticContent))
(SubClassOf :SyntheticImage :SyntheticContent)
(SubClassOf :SyntheticAudio :SyntheticContent)
(SubClassOf :SyntheticVideo :SyntheticContent)
(SubClassOf :SyntheticText :SyntheticContent)

;; Labelling Requirements
(SubClassOf :SyntheticContentGenerationAI
  (ObjectSomeValuesFrom :requiresLabelling :MachineReadableFormat))

(SubClassOf :SyntheticContentGenerationAI
  (ObjectSomeValuesFrom :requiresLabelling :HumanPerceivableFormat))

;; Artistic Exception
(Declaration (Class :ArtisticSyntheticContent))
(SubClassOf :ArtisticSyntheticContent :SyntheticContentGenerationAI)

(SubClassOf :ArtisticSyntheticContent
  (ObjectSomeValuesFrom :serves :ArtisticPurpose))

(SubClassOf :ArtisticSyntheticContent
  (ObjectSomeValuesFrom :serves :CreativePurpose))

(SubClassOf :ArtisticSyntheticContent
  (ObjectSomeValuesFrom :serves :SatiricalPurpose))

(SubClassOf :ArtisticSyntheticContent
  (ObjectSomeValuesFrom :requires :AppropriateSafeguards))

;; Actor Obligations
(Declaration (Class :ProviderTransparencyDuty))
(SubClassOf :LimitedRiskAI
  (ObjectSomeValuesFrom :obligates :ProviderTransparencyDuty))

(Declaration (Class :DeployerDisclosureDuty))
(SubClassOf :LimitedRiskAI
  (ObjectSomeValuesFrom :obligates :DeployerDisclosureDuty))

;; Data Properties
(DataPropertyAssertion :hasLegalBasis :LimitedRiskAI
  "EU Regulation 2024/1689 Chapter IV, Article 50"^^xsd:string)

(DataPropertyAssertion :hasRiskLevel :LimitedRiskAI
  "limited"^^xsd:string)

(DataPropertyAssertion :hasEnforcementDate :LimitedRiskAI
  "2026-08-02"^^xsd:date)

(DataPropertyAssertion :hasMaximumFine :LimitedRiskAI
  "€7.5 million or 1.5% of global annual turnover"^^xsd:string)

;; Annotations
(AnnotationAssertion rdfs:label :LimitedRiskAI
  "Limited Risk AI System"@en)
(AnnotationAssertion rdfs:comment :LimitedRiskAI
  "AI systems subject only to transparency obligations, including chatbots, emotion recognition, biometric categorisation, and deepfake generation. Users must be informed of AI interaction."@en)
(AnnotationAssertion :regulatorySource :LimitedRiskAI
  "EU AI Act Chapter IV, Article 50"^^xsd:string)
(AnnotationAssertion :ontologyID :LimitedRiskAI "AI-0120"^^xsd:string)
```
