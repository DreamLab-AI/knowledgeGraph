...
- ### Content
  ...
  **Quantum CLT and non-commutative probability**: Quantum probability theory (von Neumann algebras, Wigner semicircle law in random matrix theory, free probability) has its own CLT analogues. The free CLT replaces the Gaussian with the semicircle distribution; the quantum CLT for quantum observables in large quantum systems converges to a Gaussian under appropriate conditions. The 2026 abstract categorical CLT (arXiv:2603.08266) attempts to unify classical, free, and quantum CLTs within a single category-theoretic framework, representing the mathematical frontier of CLT generalisation.
  ...
  ## CLT in Financial Mathematics and Risk

  The CLT plays a foundational role in [[Financial Mathematics]] and quantitative risk management, though the tension between CLT validity and the heavy-tailed reality of financial returns is a recurring theme:

  **Portfolio diversification**: The fundamental principle of portfolio diversification is a direct application of the CLT. If individual asset returns Rᵢ are approximately independent with mean μ and standard deviation σ, the average portfolio return R̄ₙ = (1/n) ∑ Rᵢ has the same mean μ but standard deviation σ/√n — the CLT guarantees that diversifying across n assets reduces idiosyncratic risk by √n. This insight, formalised in Markowitz's Modern Portfolio Theory (1952), remains the cornerstone of institutional asset allocation and is the mathematical basis for mutual funds and ETFs.

  **Value-at-Risk under normality**: The most widely used market risk measure, Value-at-Risk (VaR), is defined as the loss L* such that P(loss > L*) = α (typically α = 1% or 5%). Under the CLT normal approximation, VaR = μ + z_α · σ where z_α is the α-quantile of the standard normal. This formula is implemented in virtually every bank's trading book risk system. However, the empirical return distribution has heavier tails than the normal (excess kurtosis in equity returns is typically 4–10), meaning CLT-based VaR systematically underestimates tail risk. The 2008 financial crisis exposed this limitation dramatically, motivating the shift toward Expected Shortfall (ES, also called CVaR) — the expected loss conditional on exceeding VaR — as the regulatory measure under Basel III/IV.

  **Options pricing under Black-Scholes**: The Black-Scholes model (1973) assumes stock prices follow geometric Brownian motion, which implies that log-returns over any fixed period are Gaussian — a consequence of the CLT applied to the sum of many small independent log-returns. The Black-Scholes formula for European option prices follows analytically from this Gaussian assumption. The model's practical failure — the "volatility smile" where implied volatility varies with strike, inconsistent with the flat smile predicted by geometric Brownian motion — reflects the empirical heavy-tailedness of returns and has motivated extensions to stochastic volatility, jump-diffusion, and Lévy process models that relax the CLT's Gaussian limit.

  **Actuarial applications**: Insurance and reinsurance pricing rests on the CLT's application to claims distributions. For a portfolio of n independent policies, the CLT guarantees that the aggregate claims S_n = ∑ Xᵢ is approximately Gaussian for large n, enabling the calculation of the premium loading (safety margin above expected claims) required to ensure solvency with a target probability. The Solvency II regulatory framework in Europe and the PRA's UK variant require insurers to hold capital sufficient to survive a 1-in-200 year loss — a 99.5th percentile calculation that uses CLT normal approximations for well-diversified portfolios but requires heavier-tailed distributions (Pareto, generalised Pareto, log-normal) for catastrophic event risks.
