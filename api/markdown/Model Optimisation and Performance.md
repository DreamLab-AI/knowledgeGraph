...
- ### Content
  ...
  - #### DistilBERT
  - **DistilBERT** (Sanh et al. HuggingFace 2019) demonstrated that a 6-layer 66M parameter student [private] retain 97% of BERT-base 12-layer 110M's GLUE performance by combining (i) soft target distillation, (ii) cosine embedding loss on hidden states, and (iii) masked language modelling loss, trained on the same corpus as BERT using a linear combination. DistilBERT runs 60% faster than BERT-base and remains one of the most downloaded models on Hugging Face (500M+ downloads).
