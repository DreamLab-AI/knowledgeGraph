id:: userconsenttoken-ontology

- ### OntologyBlock
  id:: User Consent Token
  - ontology:: true
  - public-access:: true
  - term-id:: DT-0274
  - preferred-term:: User Consent Token
  - source-domain:: bc
  - owl:class:: bc:UserConsentToken
  - status:: draft
  - definition:: A cryptographically verifiable digital token that represents and enforces user consent for data processing, collection, sharing, or participation in virtual environments with granular permission controls and revocation mechanisms.

### Relationships
- is-subclass-of:: [[Metaverse]]
- is-part-of:: [[Consent Management Framework]]
- has-part:: [[Timestamp]]
- has-part:: [[Revocation Mechanism]]
- has-part:: [[Cryptographic Signature]]
- has-part:: [[Consent Payload]]
- has-part:: [[Scope Definition]]
- requires:: [[Digital Identity]]
- requires:: [[Privacy Policy]]
- requires:: [[Consent Registry]]
- requires:: [[Cryptographic Key]]
- requires:: [[Data Schema]]
- enables:: [[Granular Consent Control]]
- enables:: [[Consent Revocation]]
- enables:: [[User Data Sovereignty]]
- enables:: [[Automated Privacy Compliance]]
- enables:: [[Consent Audit Trail]]
- depends-on:: [[Verifiable Credential]]
- depends-on:: [[Time Oracle]]
- depends-on:: [[Decentralized Identifier (DID)]]
- depends-on:: [[Blockchain Ledger]]
