...
```clojure
(Declaration (Class :PPO))
(SubClassOf :PPO :ReinforcementLearningAlgorithm)
(SubClassOf :PPO
  (ObjectSomeValuesFrom :updates :PolicyThroughIncrementalSteps))
(SubClassOf :PPO
  (ObjectSomeValuesFrom :constrains :PolicyChanges))
(SubClassOf :PPO
  (ObjectSomeValuesFrom :prevents :DestabilizingUpdates))
(SubClassOf :PPO
  (ObjectSomeValuesFrom :uses :ClippedObjective))
(SubClassOf :PPO
  (ObjectSomeValuesFrom :standardAlgorithmFor :RLHF))
(SubClassOf :PPO
  (ObjectSomeValuesFrom :optimizes :LanguageModelPolicy))

(AnnotationAssertion rdfs:comment :PPO
  "Reinforcement learning algorithm updating policies through incremental steps whilst constraining changes to prevent destabilizing updates, standard for RLHF"@en)
(AnnotationAssertion :hasAcademicSource :PPO
  "Schulman et al. (2017); applied to RLHF in Ouyang et al., arXiv:2203.02155 (2022)")
```
...
# Proximal Policy Optimisation
		  ...
		  **Term ID**: AI-0265
		  **Category**: Training Technique / Algorithm
