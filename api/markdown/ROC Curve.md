...
```clojure
;; Class Declaration
(Declaration (Class :ROCCurve))
(SubClassOf :ROCCurve :PerformanceMetric)
(SubClassOf :ROCCurve :VisualisationTool)
(SubClassOf :ROCCurve :ThresholdIndependentMetric)

;; Full name
(Declaration (Class :ReceiverOperatingCharacteristicCurve))
(EquivalentClasses :ROCCurve :ReceiverOperatingCharacteristicCurve)

;; Core relationships
(SubClassOf :ROCCurve
  (ObjectSomeValuesFrom :evaluatesPerformanceOf :BinaryClassifier))
(SubClassOf :ROCCurve
  (ObjectSomeValuesFrom :summarisedBy :AUC))
(SubClassOf :ROCCurve
  (ObjectSomeValuesFrom :alternativeTo :PrecisionRecallCurve))
(SubClassOf :ROCCurve
  (ObjectSomeValuesFrom :enablesSelection :ClassificationThreshold))

;; Axes definition
(Declaration (DataProperty :xAxisMetric))
(Declaration (DataProperty :yAxisMetric))
(DataPropertyAssertion :xAxisMetric :ROCCurve "False Positive Rate"^^xsd:string)
(DataPropertyAssertion :yAxisMetric :ROCCurve "True Positive Rate"^^xsd:string)

;; Curve components
(Declaration (ObjectProperty :plotsMetric))
(SubClassOf :ROCCurve
  (ObjectIntersectionOf
    (ObjectSomeValuesFrom :plotsMetric :TruePositiveRate)
    (ObjectSomeValuesFrom :plotsMetric :FalsePositiveRate)))

;; Key points
(Declaration (Class :PerfectClassifierPoint))
(Declaration (Class :RandomClassifierLine))
(Declaration (ObjectProperty :hasKeyPoint))

(SubClassOf :ROCCurve
  (ObjectSomeValuesFrom :hasKeyPoint :PerfectClassifierPoint))

(AnnotationAssertion rdfs:comment :PerfectClassifierPoint
  "Point (0, 1) representing perfect classification: all true positives, no false positives"@en)
(AnnotationAssertion rdfs:comment :RandomClassifierLine
  "Diagonal line (y=x) representing random guessing baseline"@en)

;; Threshold independence
(Declaration (Class :ThresholdIndependentEvaluation))
(SubClassOf :ROCCurve :ThresholdIndependentEvaluation)

;; Area under curve
(Declaration (ObjectProperty :hasAreaUnderCurve))
(SubClassOf :ROCCurve
  (ObjectSomeValuesFrom :hasAreaUnderCurve :AUC))

;; Curve interpretation
(Declaration (DataProperty :curveInterpretation))
(DataPropertyDomain :curveInterpretation :ROCCurve)
(DataPropertyRange :curveInterpretation xsd:string)
(AnnotationAssertion rdfs:comment :curveInterpretation
  "Higher and left curves indicate better performance; curves above diagonal better than random"@en)

;; Use cases
(Declaration (Class :ModelComparison))
(Declaration (Class :ThresholdSelection))
(Declaration (ObjectProperty :supports))
(SubClassOf :ROCCurve
  (ObjectIntersectionOf
    (ObjectSomeValuesFrom :supports :ModelComparison)
    (ObjectSomeValuesFrom :supports :ThresholdSelection)))

;; Annotations
(AnnotationAssertion rdfs:label :ROCCurve "ROC Curve"@en)
(AnnotationAssertion rdfs:label :ROCCurve "Receiver Operating Characteristic Curve"@en)
(AnnotationAssertion rdfs:comment :ROCCurve
  "A graphical performance evaluation tool for binary classification models that plots the True Positive Rate against the False Positive Rate across all possible classification thresholds."@en)
(AnnotationAssertion dcterms:source :ROCCurve <https://www.iso.org/standard/74438.html>)
(AnnotationAssertion :termID :ROCCurve "AI-0112"^^xsd:string)

;; Historical origin
(AnnotationAssertion :historicalOrigin :ROCCurve
  "Signal detection theory, World War II radar operator performance analysis"^^xsd:string)

;; ISO/IEC alignment
(AnnotationAssertion :alignedWith :ROCCurve :ISO25059)
(AnnotationAssertion :alignedWith :ROCCurve :NISTAIRFM_MEASURE22)
```
