...
- ### Content
  ...
  - The speed of ControlNet's adoption is historically unusual for a research contribution: the GitHub repository reached 10,000 stars within weeks of the February 2023 preprint, a rate of community adoption comparable only to the original [[Stable Diffusion Image Model]] release and the AUTOMATIC1111 WebUI. This rapid uptake was driven by the practical impact—users [private] immediately achieve controllable character posing, edge-guided photorealism, and depth-conditioned scene composition using community-trained checkpoint weights released alongside the code—combined with the relatively modest hardware requirements (ControlNet for SD 1.5 operates on 6 GB consumer GPU VRAM) that democratised access across the hobbyist, indie developer, and professional user base simultaneously.
