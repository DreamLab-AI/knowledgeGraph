...
# Overfitting
		  ...
		  - **Term ID**: AI-0054
		  - **Type**: AIRisk
		  - **Classification**: Training Problem
		  - **Domain**: MLDomain
		  ...
		  **Source**: ISO/IEC 22989:2022 (Training context) + NIST AI 100-3 - Authority Score: 0.95
		  
		  ### Operational Characteristics
		  - **Training Performance**: Excellent on training data
		  - **Generalisation Failure**: Poor performance on test/validation data
		  - **Memorisation**: Model memorises rather than learns patterns
		  - **Complexity Issue**: Often due to excessive model capacity
		  - **Validation Gap**: Large difference between train and validation loss
		  
		  ## Relationships
		  
		  ### Parent Classes
		  - **Training Problem**: Overfitting is a training pathology
		  - **AIRisk**: Represents model quality risk
		  ...
		  ```clojure
		  ;; Overfitting Ontology (OWL Functional Syntax)
		  ;; Term ID: AI-0054
		  ;; Domain: MLDomain | Layer: AlgorithmicLayer
		  
		  (Declaration (Class :Overfitting))
		  
		  ;; Core Classification
		  (SubClassOf :Overfitting :TrainingProblem)
		  (SubClassOf :Overfitting :AIRisk)
		  
		  ;; Performance Characteristics
		  (SubClassOf :Overfitting
		    (ObjectSomeValuesFrom :exhibitsHigh :TrainingPerformance))
		  (SubClassOf :Overfitting
		    (ObjectSomeValuesFrom :exhibitsLow :TestPerformance))
		  (SubClassOf :Overfitting
		    (ObjectSomeValuesFrom :failsAt :Generalisation))
		  
		  ;; Causality
		  (SubClassOf :Overfitting
		    (ObjectSomeValuesFrom :causedByExcessive :ModelCapacity))
		  (SubClassOf :Overfitting
		    (ObjectSomeValuesFrom :resultOfMemorising :TrainingData))
		  
		  ;; Prevention
		  (SubClassOf :Overfitting
		    (ObjectSomeValuesFrom :preventedBy :Regularisation))
		  (SubClassOf :Overfitting
		    (ObjectSomeValuesFrom :detectedUsing :ValidationData))
		  
		  ;; Contrast
		  (DisjointClasses :Overfitting :Underfitting)
		  
		  ;; Annotations
		  (AnnotationAssertion rdfs:label :Overfitting "Overfitting"@en-GB)
		  (AnnotationAssertion rdfs:comment :Overfitting
		    "Phenomenon where model learns training data too well including noise, resulting in poor generalisation"@en)
		  (AnnotationAssertion :isoReference :Overfitting "ISO/IEC 22989:2022")
		  (AnnotationAssertion :isoRiskReference :Overfitting "ISO/IEC 23894:2023")
		  (AnnotationAssertion :authorityScore :Overfitting "0.95"^^xsd:float)
		  (AnnotationAssertion :priorityLevel :Overfitting "1"^^xsd:integer)
		  
		  ;; Data Properties
		  (DataPropertyAssertion :highTrainingPerformance :Overfitting "true"^^xsd:boolean)
		  (DataPropertyAssertion :lowTestPerformance :Overfitting "true"^^xsd:boolean)
		  (DataPropertyAssertion :indicatesMemorisation :Overfitting "true"^^xsd:boolean)
		  ```
		  </details>
