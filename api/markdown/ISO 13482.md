public:: true

# ISO 13482
```json-ld
{
  "@context": "https://narrativegoldmine.com/context/v1.jsonld",
  "@id": "urn:visionflow:page:iso-13482",
  "@type": "Page",
  "vc:slug": "iso-13482",
  "title": "ISO 13482",
  "vc:public": true,
  "vc:outboundWikilinks": [],
  "vc:schemaVersion": 2
}
```

```json-ld
{
  "@context": "https://narrativegoldmine.com/ns/v2.jsonld",
  "@id": "urn:ngm:class:iso-13482",
  "@type": "Class",
  "label": "ISO 13482",
  "definition": "ISO 13482 is the international standard specifying safety requirements for personal care robots—autonomous or semi-autonomous robots designed to perform tasks that directly assist humans in daily life, including mobility assistance, physical assistance, and person-carrier applications. Published in 2014, the standard defines risk assessment methodologies, protective measure requirements, and verification procedures tailored to the unique hazards of robots operating in close physical proximity to vulnerable populations such as the elderly and disabled. It complements the industrial robot safety standard ISO 10218 and is increasingly referenced in regulatory frameworks for assistive and service robotics.",
  "domain": "robotics",
  "maturity": "established",
  "subClassOf": [
    {"@id": "urn:ngm:class:functional-safety", "label": "Functional Safety"}
  ],
  "relations": {
    "relatedTo": [
      {"@id": "urn:ngm:class:robot-safety", "label": "Robot Safety"},
      {"@id": "urn:ngm:class:iso-10218", "label": "ISO 10218"},
      {"@id": "urn:ngm:class:cobot-safety-levels", "label": "Cobot Safety Levels"},
      {"@id": "urn:ngm:class:assistive-robotics", "label": "Assistive Robotics"},
      {"@id": "urn:ngm:class:service-robot", "label": "Service Robot"},
      {"@id": "urn:ngm:class:iso-12100", "label": "ISO 12100"},
      {"@id": "urn:ngm:class:hazard-identification", "label": "Hazard Identification"},
      {"@id": "urn:ngm:class:safety-case", "label": "Safety Case"}
    ],
    "uses": [
      {"@id": "urn:ngm:class:risk-assessment", "label": "Risk Assessment"},
      {"@id": "urn:ngm:class:rb-0088-iso-13482-compliance", "label": "rb 0088 iso 13482 compliance"},
      {"@id": "urn:ngm:class:risk-estimation", "label": "Risk Estimation"},
      {"@id": "urn:ngm:class:hazard-analysis", "label": "Hazard Analysis"}
    ],
    "enables": [
      {"@id": "urn:ngm:class:human-robot-interaction", "label": "Human Robot Interaction"},
      {"@id": "urn:ngm:class:autonomous-system", "label": "Autonomous System"},
      {"@id": "urn:ngm:class:robot-certification", "label": "Robot Certification"},
      {"@id": "urn:ngm:class:elder-care-robot", "label": "Elder Care Robot"}
    ],
    "standardizedBy": [
      {"@id": "urn:ngm:class:iso", "label": "ISO"},
      {"@id": "urn:ngm:class:international-electrotechnical-commission", "label": "International Electrotechnical Commission"}
    ],
    "hasPart": [
      {"@id": "urn:ngm:class:protective-measure", "label": "Protective Measure"},
      {"@id": "urn:ngm:class:safety-requirement", "label": "Safety Requirement"},
      {"@id": "urn:ngm:class:verification-procedure", "label": "Verification Procedure"}
    ],
    "requires": [
      {"@id": "urn:ngm:class:safety-engineering", "label": "Safety Engineering"},
      {"@id": "urn:ngm:class:contact-force-limit", "label": "Contact Force Limit"}
    ],
    "supports": [
      {"@id": "urn:ngm:class:exoskeleton", "label": "Exoskeleton"},
      {"@id": "urn:ngm:class:rehabilitation-robot", "label": "Rehabilitation Robot"},
      {"@id": "urn:ngm:class:powered-wheelchair", "label": "Powered Wheelchair"}
    ],
    "bridgesTo": [
      {"@id": "urn:ngm:class:governance", "label": "Governance"},
      {"@id": "urn:ngm:class:machine-learning", "label": "Machine Learning"}
    ]
  },
  "sameAs": [
    {"@id": "urn:ngm:class:personal-care-robot-safety-standard", "label": "Personal Care Robot Safety Standard"}
  ],
  "quality": 0.8,
  "provenance": {
    "attributedTo": "did:nostr:ontology-mesh",
    "generatedAt": "2026-06-14T00:00:00Z",
    "inferenceRule": "RelationEnrichment"
  }
}
```

- ### Definition
  - [[ISO 13482]] is the international standard governing safety requirements for personal care robots that interact directly with humans, specifying [[Risk Assessment]] methodologies and protective measures to enable safe [[Human Robot Interaction]] in [[Assistive Robotics]] and [[Service Robot]] applications.
- ### Relationships
  - ISO 13482 is a specialisation of [[Functional Safety]] standards applied to robotics. It uses structured [[Risk Assessment]] procedures and is operationalised through compliance artefacts such as [[rb 0088 iso 13482 compliance]]. It relates to the industrial counterpart [[ISO 10218]] and addresses safety levels analogous to [[Cobot Safety Levels]] for the personal care context. It governs the deployment of [[Assistive Robotics]] and [[Service Robot]] systems, enabling safe [[Human Robot Interaction]] and the certification of [[Autonomous System]] behaviours in domestic and healthcare settings.
- ### Content
  - ISO 13482 emerged from the recognition that existing industrial robot safety standards, primarily ISO 10218, were ill-suited to robots designed to work alongside or physically support human occupants in unstructured domestic and care environments. Industrial standards assume a clear physical separation between robot and human, whereas personal care robots must routinely touch, lift, and guide people, requiring a fundamentally different safety philosophy centred on contact force limits, fall prevention, and behavioural predictability.

  - The standard distinguishes three principal application types: mobile servant robots that carry objects for humans, physical assistant robots that guide or support body movement, and person-carrier robots such as powered wheelchairs and transfer aids. Each category carries distinct hazard profiles that demand tailored risk reduction measures. For physical assistant robots, particular attention is paid to load path analysis, actuator failure modes, and emergency stop responsiveness given that failures may directly injure the person being assisted.

  - The risk assessment methodology mandated by ISO 13482 follows the iterative hazard identification, risk estimation, and risk evaluation loop established in ISO 12100. Designers must enumerate use cases and foreseeable misuse scenarios, estimate severity and probability for each hazardous situation, and demonstrate that residual risk after protective measures is acceptable. Verification typically combines analysis, simulation, prototype testing, and user trials, with documentary evidence maintained in a safety case.

  - As humanoid robots and exoskeleton devices approach commercial maturity, ISO 13482 compliance is emerging as a de facto market entry requirement in markets such as Japan and the European Union. Robotics companies developing domestic elder-care robots, rehabilitation exoskeletons, and patient-transfer devices are investing in early-stage safety engineering to accelerate certification timelines. The standard is also under revision to address AI-enabled adaptive behaviours that were not envisaged in the 2014 edition.
