...
```clojure
;; Class Declaration
(Declaration (Class :ConfusionMatrix))
(SubClassOf :ConfusionMatrix :PerformanceMetric)
(SubClassOf :ConfusionMatrix :VisualisationTool)
(SubClassOf :ConfusionMatrix :AnalyticalTool)

;; Core relationships
(SubClassOf :ConfusionMatrix
  (ObjectSomeValuesFrom :evaluatesPerformanceOf :ClassificationModel))
(SubClassOf :ConfusionMatrix
  (ObjectSomeValuesFrom :produces :Accuracy))
(SubClassOf :ConfusionMatrix
  (ObjectSomeValuesFrom :produces :Precision))
(SubClassOf :ConfusionMatrix
  (ObjectSomeValuesFrom :produces :Recall))
(SubClassOf :ConfusionMatrix
  (ObjectSomeValuesFrom :produces :F1Score))
(SubClassOf :ConfusionMatrix
  (ObjectSomeValuesFrom :produces :Specificity))

;; Matrix components
(Declaration (Class :TruePositive))
(Declaration (Class :TrueNegative))
(Declaration (Class :FalsePositive))
(Declaration (Class :FalseNegative))
(Declaration (ObjectProperty :hasMatrixCell))

(SubClassOf :ConfusionMatrix
  (ObjectIntersectionOf
    (ObjectSomeValuesFrom :hasMatrixCell :TruePositive)
    (ObjectSomeValuesFrom :hasMatrixCell :TrueNegative)
    (ObjectSomeValuesFrom :hasMatrixCell :FalsePositive)
    (ObjectSomeValuesFrom :hasMatrixCell :FalseNegative)))

;; Data properties for cell counts
(Declaration (DataProperty :truePositiveCount))
(DataPropertyDomain :truePositiveCount :ConfusionMatrix)
(DataPropertyRange :truePositiveCount xsd:integer)

(Declaration (DataProperty :trueNegativeCount))
(DataPropertyDomain :trueNegativeCount :ConfusionMatrix)
(DataPropertyRange :trueNegativeCount xsd:integer)

(Declaration (DataProperty :falsePositiveCount))
(DataPropertyDomain :falsePositiveCount :ConfusionMatrix)
(DataPropertyRange :falsePositiveCount xsd:integer)

(Declaration (DataProperty :falseNegativeCount))
(DataPropertyDomain :falseNegativeCount :ConfusionMatrix)
(DataPropertyRange :falseNegativeCount xsd:integer)

;; Matrix dimensions
(Declaration (DataProperty :numberOfClasses))
(DataPropertyDomain :numberOfClasses :ConfusionMatrix)
(DataPropertyRange :numberOfClasses xsd:integer)
(AnnotationAssertion rdfs:comment :numberOfClasses
  "Number of classes in classification problem (2 for binary, n for multi-class)"@en)

;; Normalisation types
(Declaration (Class :UnnormalisedMatrix))
(Declaration (Class :RowNormalisedMatrix))
(Declaration (Class :ColumnNormalisedMatrix))
(Declaration (Class :GlobalNormalisedMatrix))
(SubClassOf :UnnormalisedMatrix :ConfusionMatrix)
(SubClassOf :RowNormalisedMatrix :ConfusionMatrix)
(SubClassOf :ColumnNormalisedMatrix :ConfusionMatrix)
(SubClassOf :GlobalNormalisedMatrix :ConfusionMatrix)

;; Capabilities
(Declaration (Class :ErrorPatternAnalysis))
(Declaration (ObjectProperty :enables))
(SubClassOf :ConfusionMatrix
  (ObjectSomeValuesFrom :enables :ErrorPatternAnalysis))

;; Annotations
(AnnotationAssertion rdfs:label :ConfusionMatrix "Confusion Matrix"@en)
(AnnotationAssertion rdfs:comment :ConfusionMatrix
  "A tabular visualisation and analytical tool summarising the performance of a classification model by displaying the counts or proportions of predictions cross-tabulated against actual class labels."@en)
(AnnotationAssertion dcterms:source :ConfusionMatrix <https://www.iso.org/standard/74438.html>)
(AnnotationAssertion :termID :ConfusionMatrix "AI-0111"^^xsd:string)

;; ISO/IEC alignment
(AnnotationAssertion :alignedWith :ConfusionMatrix :ISO25059)
(AnnotationAssertion :alignedWith :ConfusionMatrix :NISTAIRFM_MEASURE22)
```
...
- [[Specificity]]
	...
	- #### Relationships
	  id:: confusion-matrix-relationships
		- enables:: [[Error analysis]], [[fairness assessment]], [[model debugging]]
