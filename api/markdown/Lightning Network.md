...
- ### Content
  ...
  - **Channel Factories via Ark/Spark/covenants**: The convergence of Ark-style virtual UTXOs with Lightning's payment graph [private] enable mass onboarding of Lightning users at a fraction of the current per-user on-chain footprint. If Bitcoin adds covenant-enabling opcodes (OP_CAT, CTV, OP_VAULT) through a future soft fork, programmable channel factories become feasible, potentially reducing the on-chain cost of onboarding 1 million Lightning users to the equivalent of onboarding 10,000 users via direct channel opens — a 100x efficiency gain enabling Lightning to reach true global consumer scale.
