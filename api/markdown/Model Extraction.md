...
(Declaration (Class :ModelExtraction))

;; Annotations
(AnnotationAssertion rdfs:label :ModelExtraction "Model Extraction"@en)
(AnnotationAssertion rdfs:comment :ModelExtraction "An attack where adversaries reconstruct a functionally equivalent or similar machine learning model by systematically querying a target model and training a substitute model on the collected input-output pairs, enabling theft of intellectual property, privacy violations, and subsequent attacks."@en)

;; Data Properties
(AnnotationAssertion dcterms:identifier :ModelExtraction "AI-0090"^^xsd:string)
(DataPropertyAssertion :isAITechnology :ModelExtraction "true"^^xsd:boolean)
```
