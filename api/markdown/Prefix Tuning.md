...
```clojure
(Declaration (Class :PrefixTuning))
(SubClassOf :PrefixTuning :ParameterEfficientFineTuning)
(SubClassOf :PrefixTuning
  (ObjectSomeValuesFrom :prepends :TrainablePrefixes))
(SubClassOf :PrefixTuning
  (ObjectSomeValuesFrom :modifies :KeyValueMatrices))
(SubClassOf :PrefixTuning
  (ObjectSomeValuesFrom :affects :AttentionComputation))
(SubClassOf :PrefixTuning
  (ObjectSomeValuesFrom :appliesAtEach :TransformerLayer))
(SubClassOf :PrefixTuning
  (ObjectSomeValuesFrom :freezes :PreTrainedParameters))
(SubClassOf :PrefixTuning
  (DataPropertyAssertion :hasParameterFraction "~0.1%"))

(AnnotationAssertion rdfs:comment :PrefixTuning
  "Parameter-efficient fine-tuning prepending trainable continuous vectors to key and value matrices at each transformer layer"@en)
(AnnotationAssertion :hasAcademicSource :PrefixTuning
  "PEFT research arXiv:2312.12148 (2023)")
```
