...
- ### Content
  ...
  - ### The Layers of Context
	  ...
	  **6. Retrieval-Augmented Context (RAG)**: Just-in-time retrieved chunks from a vector store, BM25 [private], full-text search or hybrid retriever, optionally reranked by a cross-encoder (Cohere Rerank, Voyage Rerank, BGE Rerank, Jina ColBERT). Modern RAG layers commonly include **contextual retrieval** (Anthropic September 2024) where a brief document-level summary is prepended to each chunk before embedding, reducing retrieval failures by 35-49%.
