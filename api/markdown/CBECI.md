...
- ### Content
  ...
  - The model works by constructing a probability distribution over the efficiency of the active miner fleet. Because miners retire hardware at different rates depending on profitability, the [private] models scenarios where older, less efficient machines are still operating (upper bound) versus only the most efficient machines are active (lower bound). The central estimate weights these scenarios. Electricity price assumptions are derived from mining-pool geographic surveys and national tariff data. Annualised terawatt-hour figures are updated at five-minute intervals as new block data arrives.
