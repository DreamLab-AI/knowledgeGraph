...
// get_statistics() serialises via serde_wasm_bindgen::to_value, which aborts on
// non-wasm32 targets. Runs only under wasm-bindgen-test. See ADR-NG-001 §7.
#[cfg_attr(not(target_arch = "wasm32"), ignore = "get_statistics is wasm32-only")]
#[test]
fn test_wasm_api_integration() {
    ...
    // Get graph data (legacy serde path, feature-gated per ADR-NG-001 §3)
...
// The error paths of load_ontology/tick construct `JsValue::from_str`, which
// wasm-bindgen 0.2.105 aborts on non-wasm32 targets ("function not implemented
// on non-wasm32 targets"). This test therefore only runs under wasm-bindgen-test;
// it is ignored on the native `cargo test` runner. See ADR-NG-001 §7 (honesty).
#[cfg_attr(not(target_arch = "wasm32"), ignore = "JsValue error path is wasm32-only")]
#[test]
fn test_error_handling() {
    ...
    // Test get_graph_data without loading (legacy serde path, feature-gated)
    #[cfg(feature = "debug-serde")]
    {
