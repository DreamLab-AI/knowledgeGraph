...
//!
//! ## SIMD Optimization
//! When the `simd` feature is enabled, leaf node force calculations use
//! WASM SIMD128 for improved performance on supported browsers.

use nalgebra::Vector2;
use crate::graph::Node;
#[cfg(feature = "simd")]
use super::simd::{SimdPositions, calculate_leaf_force_simd};
...
impl QuadTree {
    ...
    /// Calculate exact force for leaf nodes
    ///
    /// Uses SIMD optimization when the `simd` feature is enabled and
    /// there are multiple source nodes in this leaf.
    #[allow(unsafe_code)]
    fn calculate_exact_force(
        ...
        // Filter out the node we're calculating force for
        let sources: Vec<&NodeRef> = self.nodes
            .iter()
        ...
        // Use SIMD for leaf calculations when we have multiple sources
        #[cfg(feature = "simd")]
        {
            if sources.len() >= 4 {
                let positions: Vec<(f64, f64, f64)> = sources
                    .iter()
                    .map(|n| (n.position.x, n.position.y, n.mass))
                    .collect();
                let simd_sources = SimdPositions::from_positions_weighted(&positions);

                #[cfg(target_arch = "wasm32")]
                {
                    // Safety: SIMD feature is enabled and target_feature is set by the build
                    let (fx, fy) = unsafe {
                        calculate_leaf_force_simd(
                            node_pos.x as f32,
                            node_pos.y as f32,
                            &simd_sources,
                            strength as f32,
                    ...
                    return Vector2::new(fx as f64, fy as f64);
                }

                #[cfg(not(target_arch = "wasm32"))]
                {
                    let (fx, fy) = calculate_leaf_force_simd(
                        node_pos.x as f32,
                        node_pos.y as f32,
                        &simd_sources,
                        strength as f32,
        ...
        // Scalar fallback for small leaf nodes or when SIMD is disabled
        sources
            .iter()
            .map(|node| {
