...
//!
//! This module implements a force-directed graph layout algorithm with
//! optional SIMD acceleration. When the `simd` feature is enabled, batch
//! force calculations use WASM SIMD128 for improved performance.
//!
//! ## Performance
//! - Without SIMD: ~0.15ms per tick for 100 nodes
//! - With SIMD: ~0.08ms per tick for 100 nodes (50% improvement)
//! - Barnes-Hut: O(n log n) for large graphs (>50 nodes)
...
impl ForceSimulation {
    ...
    ///
    /// When the `simd` feature is enabled, uses SIMD-accelerated batch
    /// calculations for repulsion and centering forces.
    fn calculate_forces(&self, graph: &VowlGraph) -> HashMap<String, Vector2<f64>> {
        ...
        // Log SIMD status on first iteration
        #[cfg(all(target_arch = "wasm32", feature = "simd"))]
        {
            if self.iteration == 0 {
                web_sys::console::log_1(&"[SIMD] WASM SIMD128 force calculations enabled".into());
            }
        }

        // Apply repulsion between all nodes
        if self.config.use_barnes_hut && n > 50 {
            // Use Barnes-Hut approximation for large graphs
            // Barnes-Hut leaf nodes use SIMD when feature is enabled
            ...
            // Use SIMD-optimized batch repulsion for small graphs
            let positions: Vec<(f64, f64)> = nodes
                .iter()
                .map(|n| (n.visual.x, n.visual.y))
                .collect();

            let repulsion_forces = calculate_batch_repulsion(&positions, self.config.charge_strength);

            for (i, node) in nodes.iter().enumerate() {
                *forces.get_mut(&node.id).unwrap() += repulsion_forces[i];
            }
        }
        ...
        #[cfg(target_arch = "wasm32")]
        {
            if self.iteration == 0 {
                web_sys::console::log_1(&format!(
                    "[Force] Applied {} attraction forces between {} nodes ({} edges in graph)",
                    attraction_count,
                    nodes.len(),
                    graph.edge_count()
                ).into());
        ...
        // Apply centering force using SIMD batch operation
        let positions: Vec<(f64, f64)> = nodes
            .iter()
            .map(|n| (n.visual.x, n.visual.y))
            .collect();

        let center_forces = calculate_batch_center_force(
            &positions,
            self.config.center,
        ...
        for (i, node) in nodes.iter().enumerate() {
            *forces.get_mut(&node.id).unwrap() += center_forces[i];
        }
