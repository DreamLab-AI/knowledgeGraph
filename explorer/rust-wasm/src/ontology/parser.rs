...
impl StandardParser {
    ...
    fn parse_class_node(&self, json: &Value) -> Result<ClassNode> {
        ...
        let class_type = self.determine_class_type(json)?;
    ...
    /// Determine class type from JSON, including set operators
    fn determine_class_type(&self, json: &Value) -> Result<String> {
        let type_str = json
            .get("type")
            .and_then(|v| v.as_str())
            .unwrap_or("owl:Class");

        // Normalize set operator types to standard OWL format
        let class_type = match type_str {
            "owl:unionOf" | "unionOf" => "owl:unionOf",
            "owl:intersectionOf" | "intersectionOf" => "owl:intersectionOf",
            "owl:complementOf" | "complementOf" => "owl:complementOf",
            "owl:disjointUnionOf" | "disjointUnionOf" => "owl:disjointUnionOf",
            _ => type_str,
        };

        Ok(class_type.to_string())
    }
    ...
    fn parse_property(&self, json: &Value) -> Result<Property> {
        ...
        // Parse Phase 11 relationship fields
        let inverse_of = json
            .get("inverseOf")
            .or_else(|| json.get("inverse_of"))
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());

        let equivalent_to = json
            .get("equivalentProperty")
            .or_else(|| json.get("equivalent_to"))
            .and_then(|v| v.as_array())
            .map(|arr| {
                arr.iter()
                    .filter_map(|v| v.as_str())
        ...
        let disjoint_with = json
            .get("disjointWith")
            .or_else(|| json.get("disjoint_with"))
            .and_then(|v| v.as_array())
            .map(|arr| {
                arr.iter()
                    .filter_map(|v| v.as_str())
    ...
    fn test_parse_set_operators() {
        let json = r#"
        {
            "class": [
                {
                    "id": "union1",
                    "label": "Union of A and B",
                    "type": "owl:unionOf"
                },
                {
                    "id": "intersection1",
                    "label": "Intersection of C and D",
                    "type": "owl:intersectionOf"
                },
                {
                    "id": "complement1",
                    "label": "Complement of E",
                    "type": "owl:complementOf"
                },
                {
                    "id": "disjoint1",
                    "label": "Disjoint Union of F and G",
        ...
        assert_eq!(data.classes[0].class_type, "owl:unionOf");
        assert_eq!(data.classes[1].class_type, "owl:intersectionOf");
        assert_eq!(data.classes[2].class_type, "owl:complementOf");
        assert_eq!(data.classes[3].class_type, "owl:disjointUnionOf");
    }
}
