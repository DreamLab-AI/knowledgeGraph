...
//!
//! This module handles parsing OWL ontologies in JSON format and markdown format,
//! converting them into internal graph representations.

pub mod parser;
pub mod model;
pub mod markdown_parser;
pub mod owl2_validator;
pub mod loader;
...
/// Domain enumeration for the multi-ontology architecture
...
pub enum Domain {
    ...
    /// Metaverse and Virtual Worlds domain (mv:)
    #[serde(rename = "mv")]
    Metaverse,
    ...
    /// Robotics and Autonomous Systems domain (rb:)
    #[serde(rename = "rb")]
    Robotics,
    ...
    /// Blockchain and Cryptography domain (bc:)
    #[serde(rename = "bc")]
    Blockchain,
}

impl Domain {
    /// Parse domain from string prefix
    pub fn from_prefix(prefix: &str) -> Option<Self> {
        match prefix.to_lowercase().as_str() {
            "ai" | "aigo" => Some(Domain::AI),
            "mv" => Some(Domain::Metaverse),
            "tc" => Some(Domain::Telecollaboration),
            "rb" => Some(Domain::Robotics),
            "dt" => Some(Domain::DisruptiveTech),
            "bc" => Some(Domain::Blockchain),
            _ => None,
        }
    }

    /// Get the namespace prefix for this domain
    pub fn prefix(&self) -> &'static str {
        match self {
            ...
            Domain::Robotics => "rb",
            Domain::DisruptiveTech => "dt",
            Domain::Blockchain => "bc",
        }
    }

    /// Get the base URI for this domain
    pub fn base_uri(&self) -> &'static str {
        match self {
            Domain::AI => "http://narrativegoldmine.com/ai#",
            Domain::Metaverse => "http://narrativegoldmine.com/metaverse#",
            Domain::Telecollaboration => "http://narrativegoldmine.com/telecollaboration#",
            Domain::Robotics => "http://narrativegoldmine.com/robotics#",
            Domain::DisruptiveTech => "http://narrativegoldmine.com/disruptive-tech#",
            Domain::Blockchain => "http://narrativegoldmine.com/blockchain#",
        }
    }
}

/// Ontology block from markdown format (new standardized format)
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct OntologyBlock {
    /// Full IRI (e.g., "http://narrativegoldmine.com/ai#LargeLanguageModel")
    pub iri: String,

    /// Term identifier (e.g., "AI-0850")
    pub term_id: String,
    ...
    /// Core properties from Identification section
    pub core_properties: std::collections::HashMap<String, String>,

    /// Domain-specific extension properties (e.g., ai:*, bc:*)
    pub extension_properties: std::collections::HashMap<String, String>,

    /// OWL axioms in functional syntax (if present)
