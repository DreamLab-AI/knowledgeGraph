...
**High-performance Rust/WASM publisher** for Logseq knowledge graphs. This implementation provides 10x faster publishing with a 50x smaller binary compared to the legacy ClojureScript version.
...
**Architecture:**
- **Core:** Rust library (`logseq-publisher-rust/`)
- **NPM Package:** TypeScript wrapper (`logseq-publisher-npm/`)
- **Legacy:** Archived ClojureScript (`publish-spa-legacy/`) - DO NOT USE

**Performance:**
- Build time: ~2-5 seconds (vs 20-50 seconds legacy)
- Binary size: 1.1MB WASM (vs 50MB+ ClojureScript bundle)
- Memory usage: < 100MB (vs 500MB+ legacy)
